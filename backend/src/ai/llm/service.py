...
:mod:`src.ai.llm.prompts` for the single source of truth on wording.
...
from src.ai.llm.prompts import SYSTEM_PROMPT, build_user_prompt
...
class LlmService:
    ...
    def generate(self, *, question: str, context: LlmContext | None = None) -> LlmResponse:
        ...
        logger.info(
            "LLM generate model=%s rag_items=%d graph_items=%d risk_items=%d",
        ...
        raw_text = self._provider.generate(system_prompt=SYSTEM_PROMPT, user_prompt=user_prompt)
