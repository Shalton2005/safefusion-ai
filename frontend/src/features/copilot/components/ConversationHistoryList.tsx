...
import { groupConversationsByDate, formatConversationTimestamp } from '../utils/conversationGroups';
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {conversations.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {groups.map((group) => (
              <div key={group.label}>
                <p className="px-3 pb-1 text-2xs font-semibold uppercase tracking-wide text-[var(--sf-text-tertiary)]">
                  {group.label}
                </p>
                <ul className="flex flex-col gap-1" aria-label={`${group.label} conversations`}>
                        ...
                        >
                          ...
                          >
                            ...
                            <p className="truncate text-xs text-[var(--sf-text-tertiary)]">
                              {formatConversationTimestamp(conversation.updatedAt, group.label)}
                            </p>
                          </button>
                          <button
                            type="button"
