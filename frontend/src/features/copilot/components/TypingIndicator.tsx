 ...
 * slots in seamlessly above the real reply once it arrives. The bounce
 * animation is skipped under `prefers-reduced-motion` (a static dots
 * row still conveys "in progress" without motion).
 *
 * @example
...
export function TypingIndicator() {
  return (
    <div
      className="flex items-start gap-3 motion-safe:animate-fade-in"
      role="status"
      aria-label="AI Safety Copilot is typing"
    >
      <div
      ...
      <div className="rounded-xl rounded-tl-sm px-4 py-3 bg-[var(--sf-surface-raised)] border border-[var(--sf-border-default)]">
        <div className="flex items-center gap-1.5">
          {[0, 1, 2].map((i) => (
            <span
              key={i}
              className="w-1.5 h-1.5 rounded-full bg-[var(--sf-text-tertiary)] motion-safe:animate-bounce"
